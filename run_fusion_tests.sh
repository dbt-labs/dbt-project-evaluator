...
dbt build -x --target $1 --full-refresh --static-analysis=off || exit 1

echo "Running Fusion tests for the second project"
cd ../integration_tests_2
dbt deps --target $1 || exit 1
dbt seed --full-refresh --target $1 --static-analysis=off || exit 1
dbt run -x --target $1 --full-refresh --static-analysis=off || exit 1
